...
exports.getAllTouristSpots = asyncHandler(async (req, res, next) => {
  ...
  req.query.field= 'name,type,imageCover,summary,location'
  
  const feature= new APIFeatures(TouristSpot.find(),req.query).filter().limitFields().pagination();
...
exports.updateTouristSpot = asyncHandler(async (req, res, next) => {
  const touristSpot = await TouristSpot.findByIdAndUpdate(req.params.id, req.body, {
    new: true,
    runValidators: true
  });
